...
export function ProspiaMark({ size = 28, className = '' }: { size?: number; className?: string }) {
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} className={className} aria-hidden="true">
      ...
      <circle cx="50" cy="52" r="13" fill="#F5B23D" />
    </svg>
  )
}

/** Lockup horizontal: el wordmark usa currentColor (hereda el color del contenedor). */
export function ProspiaLogo({ markSize = 26, className = '' }: { markSize?: number; className?: string }) {
  return (
    <span className={`inline-flex items-center gap-2.5 ${className}`}>
      <ProspiaMark size={markSize} />
      <span
        className="font-display font-bold tracking-tight leading-none"
        style={{ fontSize: markSize * 0.92 }}
      >
        Prospia
      </span>
    </span>
